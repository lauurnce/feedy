...
_FEED_URL = "https://inferencebysequoia.substack.com/feed"


class SequoiaInferenceSource(BaseFeedSource):
    """Sequoia Inference Substack feed from inferencebysequoia.substack.com/feed."""

    @property
    def name(self) -> str:
        """Registry slug for this source."""
        return "sequoia-inference"

    def fetch(self) -> list:
        """Fetch the Substack RSS feed, returning an empty list on any transport error."""
        try:
            response = httpx.get(
                _FEED_URL,
                timeout=10,
            ...
            print(f"[SequoiaInferenceSource] fetch failed: {err}")
            return []
