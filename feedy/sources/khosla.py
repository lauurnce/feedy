...
_BASE_URL = "https://www.khoslaventures.com"
_BLOG_URL = f"{_BASE_URL}/venture-assistance-blog/"
_RSS_URL = f"{_BASE_URL}/venture-assistance-blog/feed/"


class KhoslaSource(BaseFeedSource):
    """Scrapes Khosla Ventures venture assistance blog. Requires no credentials."""
    ...
    def fetch(self) -> list:
        """Fetch blog HTML, trying RSS first."""
        try:
        ...
        try:
            ...
            print(f"[KhoslaSource] fetch failed: {err}")
            return []
...
def _parse_html(html: str, seen_urls: set) -> list[dict]:
    ...
    articles = soup.select("article, .post, .blog-post, .entry, .post-item, .content-card")
    if not articles:
        articles = soup.select("a[href*='/venture-assistance-blog/']")

    for article in articles:
        title_el = article.select_one("h1, h2, h3, h4, .title, .post-title, .entry-title, .blog-title")
